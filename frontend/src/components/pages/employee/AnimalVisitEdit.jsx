...
export default function AnimalVisitEdit() {
  ...
  async function getVisit() {
    var visit = await visitServices.getVisit(
      location.state.animalId,
      location.state.visitId
    );
    setText(visit.description);
  }
  const handleEdit = async (event) => {
    event.preventDefault();
    if (text.length >= 3) {
      visitServices
        .updateVisit(text, location.state.animalId, location.state.visitId)
        .then((response) => {
          setError("");
          navigate("/employee/allanimals/visits", {
            state: { animalId: location.state.animalId },
      ...
      <div className="pages-container">
        <div className="pages-container-info">
          <div className="pages-container-info-header">
            ...
            <h2>
              Add new visit to animal - id: {location.state.animalId},{" "}
              {location.state.visitId}{" "}
            </h2>
          </div>
